...
function Start () {
	...
	setCamera = Camera.mainCamera.gameObject;
...
function QualityUpdate() {
	if (this.enabled){
    	...
    	//check other objects
		if (unityVersionState == UNITY_VERSION.Unity){
			if (particleRingsNormalObj != null) particleRingsNormalObj.renderer.material.shader = effectShader3Ind;
			if (refPlaneObj != null) refPlaneObj.renderer.material.shader = effectShader2Ind;
			if (dropletsObj != null) dropletsObj.renderer.material.shader = effectShader3Ind;
			if (transPlaneObj != null) transPlaneObj.renderer.material.shader = effectShader3Ind;
		} else {
			if (setEffectShader != effectFull && setEffectShader != effectrefr){
				if (particleRingsNormalObj != null) particleRingsNormalObj.renderer.material.shader = effectShader3Ind;
				if (refPlaneObj != null) refPlaneObj.renderer.material.shader = setEffectShader;//effectShader2Ind;
				if (dropletsObj != null) dropletsObj.renderer.material.shader = effectShader3Ind;
				if (transPlaneObj != null) transPlaneObj.renderer.material.shader = effectShader3Ind;
			} else {
			if (setCamera.camera.renderingPath == RenderingPath.Forward){
				if (refPlaneObj != null) refPlaneObj.renderer.material.shader = effectShader2fwd;
			} else {
				if (particleRingsNormalObj != null) particleRingsNormalObj.renderer.material.shader = effectShader3;
				if (refPlaneObj != null) refPlaneObj.renderer.material.shader = setEffectShader;//effectShader2;
				if (dropletsObj != null) dropletsObj.renderer.material.shader = effectShader3;
				if (transPlaneObj != null) transPlaneObj.renderer.material.shader = effectShader3;
			}
			}
		}
