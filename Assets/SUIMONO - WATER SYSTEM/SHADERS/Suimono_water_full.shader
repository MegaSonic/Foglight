...
//RENDER REFRACTION
Pass{
	Tags {"Queue"= "Transparent-101"} //Geometry
	//LOD 400
	Cull off
	//Name "RefractiveDistortion"
	//Blend Off
	Blend SrcAlpha OneMinusSrcAlpha
	ZWrite Off
	...
	{
		_RefrSpeed *= _AnimSpeed;
		float2 effectUVs = i.uv;
		                    
		effectUVs.y += _Time * _RefrSpeed;
		effectUVs.x *= _MasterScale;
		effectUVs.y *= _MasterScale;
		float3 normal1 = UnpackNormal(tex2D(_HeightMap, effectUVs));
		                    
		effectUVs = i.uv;
		effectUVs.x *= -1;
		effectUVs.y += 0.3 + _Time * _RefrSpeed / _MasterScale;
		effectUVs.x *= _MasterScale;
		effectUVs.y *= _MasterScale;
		...
		half4 rCol = half4(tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(i.uvgrab)).rgb, 1);
